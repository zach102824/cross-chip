...
class _OGMSettingSampler:
    """Minimal OGM sampler that reads precomputed settings from a local basis file.

    The file format matches ``shadowgrouping.SettingSampler``: an (N+1) x n_qubits
    matrix where the first N rows are measurement settings (0=I, 1=X, 2=Y, 3=Z)
    and the last row is the sampling distribution.
    """
    ...
    def __init__(
        ...
        *,
        ...
        _ = (observables_int, weights, epsilon)
        data = np.loadtxt(str(ogm_file))
        if data.ndim == 1:
            data = data.reshape(1, -1)
        self.p = np.asarray(data[-1], dtype=float)
        self.p = np.maximum(self.p, 0.0)
        total = float(np.sum(self.p))
        if total <= 0.0:
            raise ValueError(f"OGM basis file has non-positive total weight: {ogm_file}")
        self.p /= total
        self.settings = np.asarray(data[:-1], dtype=int).T

    def find_setting(self, N_samples: int = 1) -> tuple[np.ndarray, dict[str, int]]:
        inds = np.random.choice(len(self.p), size=(int(N_samples),), p=self.p)
        sampled = self.settings[inds]
        if int(N_samples) == 1:
            sampled = sampled.reshape(-1)
        return sampled, {"N_samples": int(N_samples)}


def _sample_settings_from_ogm_file(
    ogm_file: str | Path,
    observables_int: np.ndarray,
    weights: np.ndarray,
    *,
    ...
    method = _OGMSettingSampler(
        observables_int,
        weights,
        ogm_file,
        epsilon=epsilon,
    )
    settings, _ = method.find_setting(N_samples=int(num_shots))
...
def estimate_energy_from_noisy_rho_shots(
    ...
    *,
    ...
    if scheme == "direct_pauli":
        ...
        sampled_settings = _sample_settings_from_ogm_file(
            ogm_file,
            observables_int,
            weights,
            num_shots=num_shots,
