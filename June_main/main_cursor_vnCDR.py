...
# Coherent (unitary) over-rotation magnitudes in radians applied per physical gate.
# 0.0 -> no coherent error (pure depolarizing, identical to the legacy behavior).
...
def _two_qubit_overrotation_unitary(eps: float, generator: str) -> np.ndarray | None:
    """Coherent over-rotation ``exp(-i * eps/2 * G)`` for a two-qubit generator ``G``.

    ``generator`` is ``"ZZ"`` (natural for CZ) or ``"ZX"`` (natural for RZX). Both
    generators square to the identity, so the exponential has the closed form
    ``cos(eps/2) I - i sin(eps/2) G`` (no matrix exponential needed). Returns ``None``
    when ``eps`` is negligibly small so no gate is inserted.
    """
    if abs(float(eps)) < 1e-15:
        return None
    z = np.array([[1.0, 0.0], [0.0, -1.0]], dtype=complex)
    x = np.array([[0.0, 1.0], [1.0, 0.0]], dtype=complex)
    second = z if generator == "ZZ" else x
    g = np.kron(z, second)
    eye4 = np.eye(4, dtype=complex)
    return np.cos(eps / 2.0) * eye4 - 1j * np.sin(eps / 2.0) * g
...
class GateArityDepolarizingNoise(cirq.NoiseModel):
    ...
    def __init__(
        ...
        *,
        ...
        coherent_2q_overrotation: float | None = None,
        coherent_1q_overrotation: float | None = None,
        coherent_cross_chip_overrotation: float | None = None,
        ...
        self.cross_chip_two_qubit_depol_prob = float(
            cross_chip_two_qubit_depol_prob
            if cross_chip_two_qubit_depol_prob is not None
            else CROSS_CHIP_TWO_QUBIT_GATE_DEPOL_PROB
        )
        # Coherent over-rotation magnitudes (radians). Cross-chip falls back to the
        # generic two-qubit value when not given explicitly.
        self.coherent_2q_overrotation = float(
            coherent_2q_overrotation
            if coherent_2q_overrotation is not None
            else TWO_QUBIT_COHERENT_OVERROTATION
        )
        self.coherent_1q_overrotation = float(
            coherent_1q_overrotation
            if coherent_1q_overrotation is not None
            else ONE_QUBIT_COHERENT_OVERROTATION
        )
        self.coherent_cross_chip_overrotation = float(
            coherent_cross_chip_overrotation
            if coherent_cross_chip_overrotation is not None
            else self.coherent_2q_overrotation
        )
    ...
    def noisy_operation(self, operation: cirq.Operation):
        ...
        if n == 2:
            # Ideal gate, then its faithful per-gate error: a coherent over-rotation
            # on the gate's natural generator followed by a stochastic depolarizing
            # channel. Each physical (e.g. folded) copy independently gets this.
            yield operation
            cross_chip = self._is_cross_chip(operation)
            generator = "ZZ" if isinstance(operation.gate, cirq.CZPowGate) else "ZX"
            eps = (
                self.coherent_cross_chip_overrotation
                if cross_chip
                else self.coherent_2q_overrotation
            )
            coherent_u = _two_qubit_overrotation_unitary(eps, generator)
            if coherent_u is not None:
                yield cirq.MatrixGate(coherent_u, name="coh2q").on(*operation.qubits)
            prob = (
        ...
        if n == 1:
            yield operation
            if abs(self.coherent_1q_overrotation) > 1e-15:
                # Systematic single-qubit over-rotation about X.
                yield cirq.rx(self.coherent_1q_overrotation).on(operation.qubits[0])
...
def scale_noise_params_for_zne(
    ...
    *,
    ...
    coherent_2q_overrotation: float = TWO_QUBIT_COHERENT_OVERROTATION,
    coherent_1q_overrotation: float = ONE_QUBIT_COHERENT_OVERROTATION,
    coherent_cross_chip_overrotation: float | None = None,
) -> dict[str, float]:
    if noise_scale <= 0:
    ...
    out = {
        ...
        "cross_chip_two_qubit_depol_prob": clip01(
            cross_chip_two_qubit_depol_prob * noise_scale
        ),
        # Coherent terms are physical over-rotations, not probabilities: they are
        # amplified by gate folding, not by scaling here, so pass them through.
        "coherent_2q_overrotation": float(coherent_2q_overrotation),
        "coherent_1q_overrotation": float(coherent_1q_overrotation),
    }
    if coherent_cross_chip_overrotation is not None:
        out["coherent_cross_chip_overrotation"] = float(coherent_cross_chip_overrotation)
    return out
...
def run_cdr_with_per_pauli_coeff_print(
    *,
    ...
    """Run vnCDR once and print the per-Pauli coefficient vectors ``a_k``.

    With vnCDR each term's fit is ``exact_k ~= a_k · x_k`` (a vector dot product over the
    folded noise levels, no intercept), so ``a_k`` is a length-``len(noise_levels)`` vector
    rather than an affine ``(a, b)`` pair. This helper is opt-in and keeps the default
    vnCDR path silent in other cells.
    """
    # Local import avoids introducing a module-level dependency cycle.
    from shot_measurement_vnCDR import run_mitigation
    ...
    noise_levels = list(models.get("noise_levels", []))
    r2_rem = np.asarray(models.get("r2_rem_to_exact_per_term", []), dtype=float)
    ...
    print(
        "vnCDR per-Pauli coefficients (exact_k ~= a_k . x_k over folded noise levels "
        f"{noise_levels}, no intercept)"
    )
    if coeffs_rem.size == 0:
        print("No per-Pauli coefficients found.")
        return out

    def _fmt_vec(vec: np.ndarray) -> str:
        return "[" + ", ".join(f"{float(v): .6f}" for v in np.atleast_1d(vec)) + "]"

    print(f"Number of Pauli terms: {len(coeffs_rem)}")
    print("REM branch coefficients:")
    for k, a_vec in enumerate(coeffs_rem):
        label = term_labels[k] if k < len(term_labels) else "UNKNOWN"
        weight = float(weights[k]) if k < len(weights) else float("nan")
        r2_k = float(r2_rem[k]) if k < len(r2_rem) else float("nan")
        print(
            f"term[{k:03d}] {label}  weight={weight: .12f}  "
            f"a={_fmt_vec(a_vec)}  R^2={r2_k: .6f}"
        )

    if coeffs_unmit.size:
        print("UNMIT branch coefficients:")
        for k, a_vec in enumerate(coeffs_unmit):
            label = term_labels[k] if k < len(term_labels) else "UNKNOWN"
            weight = float(weights[k]) if k < len(weights) else float("nan")
            print(
                f"term[{k:03d}] {label}  weight={weight: .12f}  a={_fmt_vec(a_vec)}"
            )
